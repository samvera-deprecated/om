...
We to access just the author and the advisor from the XML below.  The two <name> elements must be distinguished by their <roleTerm> value, a grandchild of the <name> element.
We like to have an :author term with value "Mary Pickral", and an :advisor term with value "David Jones".
    ...
    t.person(:path=>"name", :namespace_prefix => nil) {
      t.given(:path=>"namePart", attribtues=>{:type=>"given"})
        ...
        t.role(:namespace_prefix => nil) {
          t.text(:path=>"roleTerm", :attributes=>{:type=>"text"}, :namespace_prefix => nil)
          t.code(:path=>"roleTerm", :attributes=>{:type=>"code"}, :namespace_prefix => nil)
        }
    }
    t.author(:ref=>:person, :path=>'name[./role/roleTerm="aut"]', :namespace_prefix => nil)
    t.advisor(:ref=>:person, :path=>'name[./role/roleTerm="ths"]', :namespace_prefix => nil)
