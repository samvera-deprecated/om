...
We want an OM term to be assigned to an element's value, but only when the element does not have a specific attribute.
...
<pre>
  ...
  <element my_attr="attr value">element value</element>
  <element>element without attribute</element>
</outer> 
</pre>

we want to have an OM term named "element_plain" with "element without attribute" as the value.
...
class ExampleXMLDS < ActiveFedora::NokogiriDatastream 
  ...
  set_terminology do |t|
    t.root(:path => "outer", :xmlns => '')
    t.element(:attributes=>{:my_attr=>:none})
  end  
end # class
</pre>
